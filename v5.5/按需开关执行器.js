(() => {
  ...
  if (payload.mode === "weibo_launch") {
    const empty = { ok: 1, code: 0, data: {}, ads: [], ad: [], advertisement: [], splash: [] };
    let body = response.body || "";
    const trimmed = body.trim();
    if (!trimmed || /(?:bootpreload|bootrealtime|sdkad\.php|wbpullad\.lua|ad\/preload)/i.test(url)) {
      $done({ response: { status: 200, headers: { "Content-Type": "application/json; charset=utf-8" }, body: JSON.stringify(empty) } });
      return;
    }
    if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
      try {
        const json = JSON.parse(body);
        const cleanWeibo = value => {
          if (!value || typeof value !== "object") return value;
          if (Array.isArray(value)) {
            for (let i = value.length - 1; i >= 0; i -= 1) {
              const item = value[i];
              const text = JSON.stringify(item || {}).toLowerCase();
              if (/(广告|ad_|_ad|advert|promotion|promote|splash|uve)/i.test(text)) value.splice(i, 1);
              else cleanWeibo(item);
            }
            return value;
          }
          Object.keys(value).forEach(key => {
            const lower = key.toLowerCase();
            if (/(^|_)(ad|ads|advert|advertisement|promotion|promote|splash|launch|uve)(_|$)/.test(lower) || ["adinfo", "ad_info", "adlist", "ad_list", "cardlist_head_cards"].includes(lower)) {
              if (Array.isArray(value[key])) value[key] = [];
              else if (typeof value[key] === "number") value[key] = 0;
              else if (typeof value[key] === "boolean") value[key] = false;
              else if (value[key] && typeof value[key] === "object") value[key] = {};
              ...
              cleanWeibo(value[key]);
            }
          });
          return value;
        };
        $done({ body: JSON.stringify(cleanWeibo(json)) });
        return;
      } catch (_) {}
    }
    body = body
      .replace(/"ad(?:vertisement)?[^"]*"\s*:\s*\[[\s\S]*?\]/gi, "\"ad\":[]")
      .replace(/"splash[^"]*"\s*:\s*\[[\s\S]*?\]/gi, "\"splash\":[]")
      .replace(/"promotion[^"]*"\s*:\s*\[[\s\S]*?\]/gi, "\"promotion\":[]");
    $done({ body });
    return;
  }
